module.exports = angular.module('myServers').service('flightInfo',['server','favoriteFlight','filterFlightInfo',function(server, favoriteFlight,filterFlightInfo) {
	var that = this;
	...
	function getAllFlightInfo(params, isNewFault) {
		...
		var interfaceUrl = isNewFault ? 'maintain/meJobList' : 'flight/flightInfo';
		that.flights = [];
	...
	function getFlightinfoWithParams(params, isNewFault) {
		var interfaceUrl = isNewFault ? 'maintain/meJobList' : 'flight/searchflightInfo';
	...
	function filterFlight() {
		...
		if (params) {
				...
				filterArray = that.flights.filter(function (item) {
					if (angular.isString(params.direction)&&params.direction.length) {
						if ('departure' === params.direction) {
							return -1 != params.filterAirPorts.indexOf(item.departureAirport);
						}else if ('arrival' === params.direction) {
							return -1 != params.filterAirPorts.indexOf(item.arrivalAirport);
						}else{
							return (-1 != params.filterAirPorts.indexOf(item.departureAirport) || -1 != params.filterAirPorts.indexOf(item.arrivalAirport));
						}
					}else {
						return (-1 != params.filterAirPorts.indexOf(item.departureAirport) || -1 != params.filterAirPorts.indexOf(item.arrivalAirport));
					}
			...
			if (params.filterAbnormal.length) {
				filterArray = filterArray.filter(function (item) {
					return  -1 !=  params.filterAbnormal.indexOf(item.flightNo);
				});
			}
			if (params.filterDelayReasons.length) {
				filterArray = filterArray.filter(function (item) {
					return  -1 !=  params.filterDelayReasons.indexOf(item.abnormalType);
				});
			}
            if (params.filterAbnormal.length) {
                filterArray = filterArray.filter(function (item) {

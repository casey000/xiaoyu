...
module.exports = angular.module('myApp').controller('nrc_taskfeedbakController',
            ...
            function ($rootScope, $scope, $filter, $stateParams,server,$interval, $localForage, configInfo, b64ToBlob, $timeout) {
                ...
                function nrcdata_info(id) {
                    server.maocGetReq('maintain/getNrcOrNrcTaskFeedbackInfo/'+id).then(function(data) {
                        ...
                        if($scope.inspector.approverName!=""){
                            $scope.mrInfo.nameAndId=datainfo.inspector +" "+datainfo.inspectorNo;
                        }
                        if($scope.mrInfo.approverName == "") {
                            var params = {
                                deptName: '',//'机务',
                ...
                $scope.addcc=function () {
                    ...
                    if($scope.cc_data.type=="Pr/F" ||$scope.cc_data.type=="T/R" ||$scope.cc_data.type=="O/G"  ){
                        $rootScope.go('newcc', 'slideLeft', {defectInfo: {workorderId:$scope.cc_data.id,flightId:$scope.cc_data.preFlightId,flightNo:$scope.cc_data.preFlightNo,acReg: $scope.cc_data.acReg,acId:$scope.cc_data.acId}, docType: 'nrct', docNo: $scope.cc_data.cardNumber})

                    }else {
                        $rootScope.go('newcc', 'slideLeft', {defectInfo: {workorderId:$scope.cc_data.id,flightId:$scope.cc_data.flightId,flightNo:$scope.cc_data.flightNo,acReg: $scope.cc_data.acReg,acId:$scope.cc_data.acId}, docType: 'nrct', docNo: $scope.cc_data.cardNumber})
                    }
                };

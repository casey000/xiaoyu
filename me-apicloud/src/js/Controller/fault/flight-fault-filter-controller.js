module.exports = angular.module('myApp').controller('flightFaultFilterController', ['$rootScope', '$scope', '$location', '$localForage', '$stateParams', 'configInfo', '$filter', 'airport', 'paramTransfer', 'filterFaultFlightInfo', 'flightFaultInfo',
    function($rootScope, $scope, $location,$localForage,$stateParams,configInfo,$filter,airport,paramTransfer,filterFaultFlightInfo, flightFaultInfo) {
    ...
    $scope.filterFlight = function (params) {
        ...
        filterFaultFlightInfo.setFilters(angular.copy(params,{}));

//      meList.filterFlight();
//
//      // flightInfo.countAllFilertFlights();
//      meList.currentPage = 0;
//      $scope.noMoreData = false;
//      $scope.flights = meList.getNextPage();
    };
    ...
    function initAllFilerInfos() {
        ...
        flightFaultInfo.flights.forEach( function(item) {
    ...
    $scope.datetimePick = function (t) {
        ...
        var beginDatetime = flightFaultInfo.beginDatetime;
        var endDatetime = flightFaultInfo.endDatetime;
        if (t == startDate){
    ...
    $scope.acTypeToReg = function (event) {
        ...
        if (-1!==acTypeTabName.indexOf('757')) {
            $scope.filterArray = $scope.filterArray.filter(function (item) {
                return -1!==item.acTypeName.indexOf('B75');
            });
            $scope.filterArray.forEach(function (item) {
                $scope.acRegsArray.push(item.acRegName);
            });
        ...
        if (-1!==acTypeTabName.indexOf('767')) {
            $scope.filterArray = $scope.filterArray.filter(function (item) {
                return -1!==item.acTypeName.indexOf('B76');
            });
            $scope.filterArray.forEach(function (item) {
                $scope.acRegsArray.push(item.acRegName);
            });
        ...
        if (-1!==acTypeTabName.indexOf('747')) {
            $scope.filterArray = $scope.filterArray.filter(function (item) {
                return -1!==item.acTypeName.indexOf('B74');
            });
            $scope.filterArray.forEach(function (item) {
                $scope.acRegsArray.push(item.acRegName);
            });
    ...
    function initFilterArray() {
        ...
        $scope.acTypeTab = 1;
        $scope.searchText = '';
        var filterData = angular.copy(filterFaultFlightInfo.filterParams,{});
        $scope.startDate = new Date((flightFaultInfo.beginDatetime+parseInt(filterData.filterBeginDate))||flightFaultInfo.beginDatetime);
        $scope.endDate = new Date((flightFaultInfo.endDatetime+parseInt(filterData.filterEndDate))||flightFaultInfo.endDatetime);
        $scope.startDateLimit = $filter('date')(flightFaultInfo.beginDatetime, 'yyyy-MM-ddTHH:mm');
        $scope.endDateLimit = $filter('date')(flightFaultInfo.endDatetime, 'yyyy-MM-ddTHH:mm');
        //初始化数组

export default {
    ...
    methods: {
        ...
        getData() {
            ...
            let url = this.apiJson.getData;
            this.$httpExt().post(url, this.otherParams.id, 'gateway').then(response => {
        ...
        delRow(param) {
            ...
            this.$confirm('确认删除此行', '提示', {
                ...
                _this.$httpExt().post(this.apiJson.delRow, param.data.id, 'gateway').then(res => {
                    _this.$message({
                        type: 'success',

		...
		<el-dialog :title="'建议措施'+(showAlert==='edit'?'编辑':'新增')" :visible.sync="showDialog" width="30%" :before-close="handleClose">
			<el-form :model="formInfo" :rules="formInfoRule" ref="submitForm" label-width="100px">
				<el-form-item label="结论：" prop="result">
					<el-input v-model="formInfo.result" name="result" placeholder="请输入内容"></el-input>
				</el-form-item>
				<el-form-item label="建议措施：" prop="measures">
					<el-input type="textarea" name="measures" :rows="5" placeholder="请输入内容" v-model="formInfo.measures" resize="none">
					</el-input>
				</el-form-item>
			</el-form>
...
export default {
    ...
    mounted() {
        if(this.otherParams.method === 'view'){
            this.conSugMeasureColumn.splice(1,1)
        }
    },
	data() {
		let _this = this
		return {
		    name:'结论与建议',
            apiJson:{
                getData:apiData.ReliabilityAlarmList.sugQueryList,
                rowSubmit:apiData.ReliabilityAlarmList.sugSubmitUrl,
                delRow:apiData.ReliabilityAlarmList.sugDelUrl,
                updateSubmit:apiData.ReliabilityAlarmList.sugSureAddOrEdit,
            },
			formInfo: {
                id:'',
				warningId: '',
			...
			formInfoRule: {
				...
				measures: [{ required: true, message: '请输入延期原因', trigger: 'blur' }],
			},
			conSugMeasureColumn: [{
				headerName: '序号',
				field: "index",
			...
			{ headerName: '结论', field: "result", width: 100, suppressFilter: true },
			{ headerName: '建议措施', field: "measures", width: 100, suppressFilter: true },
			]
		}
	},
}

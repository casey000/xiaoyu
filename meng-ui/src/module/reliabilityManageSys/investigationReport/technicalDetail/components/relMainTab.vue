<template>
    <div>
        <div class="tab-title">相关维修方案：部件在维修方案中是否有检查项目：
            ...
            <el-button type="primary" @click="showDialog=true" :disabled="radio!=='Y'">增加</el-button>
        </div>
...
<!--                :multiSelect="Boolean(1)"-->
                <base-Agrid class="ag-theme-balham asms-single-action-button-grid" :columnDefs="alertTabColumn" :params="[]" actUrl="/api/v1/em/emMpdEval/queryMpList" :noFloatingFilter="true" :notShowOkCancel="true" ref="relMainTab" :enableServerSideFilter="true">
                </base-Agrid>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //维修方案措施表格查询
            mainGetData(id) {
                let url = this.apiData.mainQueryList + "?techId=" + id
            ...
            // 维修方案措施延期编辑确认
            mainRowSubmit() {
                let ids = this.$refs.relMainTab.getSelect().map(item => {
                    return {mpdId:item.evalNo,mpdNo:item.mpdNo,title:item.title}
                })
                if (ids.length === 0) return this.$message.error('请先选择方案')
                let url = this.apiData.mainAddSubmitUrl
            ...
            deleteMainRow(param) {
                this.$confirm('确认删除此行', '提示', {
                    ...
                    this.$httpExt().post(this.apiData.mainDelUrl , {mpdId:param.data.mpdId,techId:this.otherParams.id}, 'gateway').then(res => {
                        this.mainGetData(this.otherParams.id)
                        this.$message({
                            type: 'success',
                            message: '维修方案删除成功!'

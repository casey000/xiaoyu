...
<template>
	<div class="show-menu">
		...
		<el-dialog title="回顾" append-to-body :visible.sync="showHandlingTrack" custom-class="dialog-index" width="60%" :before-close="handleClose">
	...
	methods: {
		...
		resolveRow(item) {
			...
			switch (item) {
				...
				case '延期':
					...
					break;
				case '提交':
					this.$confirm('告警单提交', '提示', {
						confirmButtonText: '确定',
						cancelButtonText: '取消',
					}).then(() => {
						this.$httpExt().post('/maintenanceReliability/investigationBase/submit', { id: this.menuData.id }, 'gateway').then(res => {
							this.$message({
								type: 'success',
								message: '告警单成功提交!'
							});
		...
		extensionSubmit(formName) {
			this.$refs[formName].validate((valid) => {
				if (valid) {
					this.$httpExt().post('/maintenanceReliability/investigationBase/delaySubmit', { businessId: this.menuData.id, ...this.extensionInfo }, 'gateway').then(res => {
						this.$message({
							type: 'success',
							message: '告警单已成功延期!'
						});

...
export default {
	...
	methods: {
		...
		getMenuData(data) {
	        debugger
			this.menuData = data
            this.dealine =   formatDate(this.menuData.dueDate,'yyyy-MM-dd')
		},
		toDetail(type) {
			...
			switch (item) {
				...
				case '提交':
					this.$confirm('告警单提交', '提示', {
						...
						this.$httpExt().post('/maintenanceReliability/relWarningBase/submit', { id: this.menuData.id }, 'gateway').then(res => {
							this.$message({
								type: 'success',
								message: '告警单成功提交!'
							});
		...
		extensionSubmit(formName) {
			this.$refs[formName].validate((valid) => {
				if (valid) {
					this.$httpExt().post('/maintenanceReliability/relWarningBase/delaySubmit', { businessId: this.menuData.id, ...this.extensionInfo }, 'gateway').then(res => {
						this.$message({
							type: 'success',
							message: '告警单已成功延期!'
						});

...
var casLoginPages = utils.getEntry('./src/casLogin/*.html','other');

for (var casPathname in casLoginPages) {
    // 配置生成的html文件，定义路径等
    var conf = {
        filename: casPathname + '.html',
        template: casLoginPages[casPathname],   // 模板路径
        inject: true,              // js插入位置
        // necessary to consistently work with multiple chunks via CommonsChunkPlugin
        chunksSortMode: 'dependency',
    };
    conf.chunks = ['manifest', 'vendor', 'casLogin']
    conf.hash = true

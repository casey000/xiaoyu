...
function InitResources(){
    InitFuncCodeRequest_({
        data: {
            ...
            taskId: param.TASK_ID, flowDefId : param.FLOW_DEF_ID, projectType: param.PROJECT_TYPE,
            taskPropId: param.ACT_ID, procDefId: param.PROC_DEF_ID, procInstId: param.PROC_INST_ID
        },
        successCallBack: function (jdata) {
            ...
            if(typeof(InitLoadResource) == 'function'){
                if ('B' === param.PROJECT_TYPE) {
                    $("input[type='button']").hide();
                }
                resource['projectType'] = param.PROJECT_TYPE;
...
function doSubmit(){
    $.messager.confirm('提示', '您确定要提交吗?', function(r){
        if (r){
            ...
            if(!data.postParam)data.postParam=$("#iframe01").contents().find("#mform").serializeObject();

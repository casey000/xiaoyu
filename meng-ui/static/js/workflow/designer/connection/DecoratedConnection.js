...
draw2d.DecoratedConnection.prototype.toBpmnDI=function(){
	...
	/* @tpeng var startX = this.getSource().getAbsoluteX();
	...
	for(var i =0; i< this.lineSegments.getSize();  i++){
		var lineSeg = this.lineSegments.get(i);
		xml=xml+'<omgdi:waypoint x="'+lineSeg.start.x+'" y="'+lineSeg.start.y+'"/>\n';
		if((this.lineSegments.getSize() - 1) == i){
			xml=xml+'<omgdi:waypoint x="'+lineSeg.end.x+'" y="'+lineSeg.end.y+'"/>\n';
		}
	}
	if(this.label != null){
		xml += '<bpmndi:BPMNLabel>'+
		'<omgdc:Bounds height="'+this.label.height+'" width="'+this.label.width+'" x="'+this.label.x+'" y="'+this.label.y+'" />'+
		'</bpmndi:BPMNLabel>';
	}

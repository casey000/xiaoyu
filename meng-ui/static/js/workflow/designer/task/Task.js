...
draw2d.Task.prototype.reloadIcon=function(){
...
draw2d.Task.prototype.getContextMenu=function(){
	...
	var data = {task:this};
	menu.appendMenuItem(new draw2d.ContextMenuItem("节点属性", "properties-icon", data,function(x,y)
	{
	...
	menu.appendMenuItem(new draw2d.ContextMenuItem("删除节点", "icon-remove", data,function(x,y)
	{
		var data = this.getData();
		var task = data.task;
		var tid = task.getId();
...
draw2d.Task.prototype.getMultiInstanceXML=function(){
	var xml = '';
    if(this._loopCardinality||this.isSequential){
        xml=xml+'<multiInstanceLoopCharacteristics isSequential="'+(this.isSequential)+'" ';

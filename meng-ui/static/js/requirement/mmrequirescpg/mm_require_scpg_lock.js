...
function i18nCallBack() {
    ...
    console.log(param.data.rowData)
    ...
    InitFuncCodeRequest_({
        ...
        successCallBack: function (jdata) {
            if (jdata.code == RESULT_CODE.SUCCESS_CODE) {
                ...
                $('#lockNo').textbox({value: param.data.rowData[0].JC_NO});

                $('#partNo').combogrid({value: param.data.rowData[0].PART_NO});

                $('#unit').textbox({value: param.data.rowData[0].REQUIRE_UNIT});
...
function InitEditForm_() {
    ...
    $form.form({
        onSubmit: function () {
            ...
            data = $.extend({}, data, {
                FunctionCode: ('MM_REQUIRE_LOCK_ADD')
            });

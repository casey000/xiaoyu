...
function i18nCallBack() {
    InitFuncCodeRequest_({
        data: {domainCode: "MM_PUCHASRR_ORDER_TYPE", FunctionCode: "ANALYSIS_DOMAIN_BYCODE"},
        successCallBack: function (jdata) {
            if (jdata.code == RESULT_CODE.SUCCESS_CODE) {
                purchaseType = DomainCodeToMap_(jdata.data["MM_PUCHASRR_ORDER_TYPE"]);
                //合同类型
                $('#orderType').combobox({
                    panelHeight: '150px',
...
// W,未批准的状态 planDeadline:param.planDeadline
function InitDataGrid() {
    ...
    $("#dg").MyDataGrid({
        ...
        sortable: true,
        singleSelect: false,
        queryParams: {vendorName: vendorName, orderType: 'CG'},
        title: $.i18n.t('page:MM_OEDER_LIST.TITLE'),
        columns: {
            param: {FunctionCode: 'MM_OEDER_LIST_CG'},
            alter: {
                ORDER_TYPE: {

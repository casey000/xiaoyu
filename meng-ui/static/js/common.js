 ...
 */
var _i18n = $.i18n ? $.i18n : {t: value=>value, common_18: true};
var num = "";
var dec = "";
(function ($) {
    $.fn.extend({
    ...
    $.extend($.fn.validatebox.defaults.rules, {
        floatLength: {
            validator: function (value, param) {
                num =  param[0]-param[1];
                dec =  param[1];
                return new RegExp("^\\d{1,"+num+"}(\\.\\d{1,"+dec+"})?$","g").test(value);
            },
            message: '请输入{0}位以内数字，小数占{1}位，剩余为整数位数。'
        }
    });
    ...
    $.extend($.fn.validatebox.defaults.rules, {
        ...
        IntDeminumber: {
            validator: function (value, param) {
                return /^\d{1,14}(\.\d{1,2})?$/.test(value);
                return reg.test(value);
            },
            message: '请输入14位以内整数及2位以内小数'

        },
        numberSix: {//包含字符,ABC123,及特殊字符-或_，最大四十
            validator: function (value, param) {
        ...
        hourMin: {//小时:分
            validator: function (value, param) {
                return /^([0-9]\d*):([0-5]\d)$/.test(value);
                return reg.test(value);
            },
            message: '请输入小时:分'

        },
        timeLimit: {//输入时间不能大于当前时间
            validator: function (value) {
                var now = new Date();
        ...
        lateNow: {//输入时间不能小于当前时间
            validator: function (value) {
                var now = new Date();
                var setTime = new Date(value);
                return setTime >= now;
            },
            message: '输入时间不能小于当前时间'
        },
        phoneNum: { //验证手机号
            validator: function(value, param){
...
// 			        	return /^[a-zA-Z0-9]{4}$/i.test($.trim(value));
    ...
    /**
     * Html获取Url参数
     * 如jieshouURL.html?name=123&id=1234
     * 获取name=123,id=1234
     * 使用方式$.getUrlParam("name"),$.getUrlParam("id")
     */
    $.extend({
...
function ShowWindowIframe1(opts) {
    ...
    setTimeout(function () {
        try {
            $(SWindow.document).attr('title', _i18n.t(opts.title)? _i18n.t(opts.title): opts.title);
        } catch (e) {
            alert(e)
        }
    }, 200);
...
function ShowWindowIframe2(opts) {
    ...
    jqWindow.window({
        ...
        onMinimize: function () {
            //最下化移动到任务栏
            window.top.addTag(_i18n.t(opts.title), {
                key: win_key_,
...
function AjaxSuccess(result) {
    ...
    if (result.code == RESULT_CODE.NO_LOGIN_CODE) {
        layer.open({
            icon: 5,
            content: _i18n.t('msg_err:ERRMSG.COMMON.NOT_LOGIN'),
            yes: function (index, layero) {
                try {
                    var PWindow = getParentParam_().PWindow;
                    if (PWindow) {
                        PWindow.top.closeAllPWindows();
                        PWindow.doPost("/security/redirect", {path: "http://" + window.location.host + "/casLogin/index.html"});
                        window.close();
                    } else {
                        window.parent.location.href = "http://" + window.location.host + "/casLogin/index.html";
                    }
                } catch (e) {
                    doPost("/security/redirect", {path: "http://" + window.location.host + "/casLogin/index.html"});
                }
                layer.close(index);
            }
        });
    ...
    if (result && result.msg) {
        result.msg = _i18n.t("msg_err:" + result.msg);
...
function Ajax_(opts) {
    ...
    if(opts.server === "gateway"){
        ...
        //网关请求heads 加入token  01374973  程建波 20200701
        if(opts.path   &&  opts.path.indexOf("user/auth/login") == -1  && sessionStorage.getItem('token')){
            headers["token"] = sessionStorage.getItem('token');
        }
        var api = (opts.path.match("/.*?/.*?/")[0]);
...
function ParseFormField_(data, jQtarget, transform) {
    ...
    $.each(data, function (k, v) {
        ...
        if (jQt.length <= 0) {
            ...
            } else if (jQt.hasClass("datetimebox-f")) {  //modify: ZZ 20200617 初始化表单时自动判断进行转换
                if (isNaN(v)&&!isNaN(Date.parse(v))) {
                    jQt.datetimebox('setValue', v);
                } else {
                    jQt.datetimebox('setValue', changeDateFormat(v));
                }
            } else if (jQt.hasClass("datebox-f")) {
                if (isNaN(v)&&!isNaN(Date.parse(v))) {
                    jQt.datebox('setValue', v);
                } else {
                    jQt.datebox('setValue', changeDateFormat(v));
                }
...
function layerConfirm_1(text, btnText, yesFunc, noFunc) {
    var index_ = layer.confirm(text, {
        btn: btnText && btnText.length > 0 ? btnText : [_i18n.t('common:COMMON_OPERATION.SURE'), _i18n.t('common:COMMON_OPERATION.CANCEL')]//按钮
    }, function () {
    ...
    options = $.extend({}, options, {
        success: function (jdata) {
            if (AjaxSuccess(jdata)) {
                if(jdata && !!jdata.data &&!!jdata.data.form&&!jdata.data.form.formUrl.includes('.shtml')){
                    return window.parent.COMMONFUNLIST.openProcess(options.data);
                }
...
function layerStandardMsg_(jdata, layerIndex, successFunc_) {
    ...
    if (jdata.code == RESULT_CODE.SUCCESS_CODE) {
        MsgAlert({content: jdata.msg ? jdata.msg : _i18n.t('msg_err:ERRMSG.COMMON.SUCCESS_CODE')});
...
$(function () {
    ...
    var i18nLng = $.cookie ? $.cookie('i18next') : null;
    if(_i18n && !_i18n.common_18){
        _i18n.init({
            getAsync: false,
...
function getLoginInfo() {
    ...
    // console.log(window.sessionStorage.getItem('_LOGIN_USER'));
    // console.log(getParentParam_().PWindow.document['_LOGIN_USER']);
    // return JSON.parse(window.sessionStorage.getItem('_LOGIN_USER'));
}
 ...
 * @private
 */
function InitGatewayRequest_(opts) {
    opts.server = "gateway";
    var url = "";
    if (opts.path) {
        url = ENVGlobal.gateWay + opts.path;
    }else{
       // alert("Url is empty!");
        return;
    }
    opts = $.extend({}, opts, {url: url, success: opts.successCallBack});
    //请求数据中加入环境
    // opts.data.appKey = ENVGlobal.appKey;
    // opts.data.appSecret = ENVGlobal.appSecret;
...
/**
 * 微服务网关调用->返回参数驼峰转大写;
 *  如返回{"companyCode":"SFN"}转换后结果为{"COMPANY_CODE":"SFN"}
 * @param opts  调用参数
 * @constructor
 * @private
 */
function InitUpperGatewayRequest_(opts) {
    opts.server = "gateway";
    var url = "";
    if (opts.path) {
        url = ENVGlobal.gateWay + opts.path;
        ...
        opts, {
            ...
            success: function (jdata) {
                ...
                if (jdata.success == true) {
                    //请求数据中加入环境
                    changeToUpper(jdata.obj);
                    data = {"code":200,"data":jdata.obj};
                }else{
                    data = {"code":500,"msg":jdata.errorMessage};
                }
...
function excelExport(identity, fileName, data, jsParamfunc) {
    ...
    functionCode = (data) || opt.params.FunctionCode;
    var options = getDG(identity).datagrid('options');
    var queryParams = (data && data.queryParams) || options.queryParams;
...
/* 微服务列表标准导出 */
function excelExportMicro(identity, fileName, data, jsParamfunc) {

    var opt = getDgOpts(identity);
    functionCode = (data) || opt.params.FunctionCode;
    var options = getDG(identity).datagrid('options');
    var queryParams = (data && data.queryParams) || options.queryParams.queryParam[0];
    queryParams.page = options.pageNumber || 1;
    queryParams.rows = options.pageSize || 10;
    var res = '';
    ShowWindowIframe({
...
function ajaxLoading() {
    ...
    $("<div class=\"datagrid-mask-msg\"></div>").html("正在加载,请稍候...Waiting...").appendTo(id).css({
        display: "block",
        left: left,
        top: top
    });
...
/**
* 根据数据表格中配置，导出相应的字段
* identity: data grid 标识id
  fileName: 导出文件名
  functionCode_: 后台执行逻辑编码
  data: 参数
*/
function confFieldsExport_(identity, fileName, functionCode_, data) {
    var opt = getDgOpts(identity);
    ...
    ShowWindowIframe({
        ...
        url: '/views/confFieldsExport.shtml'
    });
}
// 分钟数转小时：分
...
/**
* param: fileElementId(文件元素id) limitSize(文件上限)
* return boolean
*/
function checkUploadFileSize(fileElementId, limitSize){
    if(!fileElementId || $("#" + fileElementId).length != 1){
        MsgAlert({content: '参数有误，无法验证文件大小', type: 'warning'});
        return;
    }

    limitSize = Number.parseInt(limitSize) > 0 ? Number.parseInt(limitSize) : 10 * 1024 * 1024; //默认为10M
    var fileSize = $("#" + fileElementId)[0].files[0].size;
...
/*
   查询列表，右键提交人主动退回工作流
 */
function wfDoReturn(objNo,objKey,dg){
    var taskId = "";
    var procDefId = "";
            ...
            InitFuncCodeRequest_({
                ...
                successCallBack: function (jdata) {
                    if (jdata.code == RESULT_CODE.SUCCESS_CODE) {
                        ...
                        procDefId = jdata.data["PROC_DEF_ID"];
                    }
                }
            });
...
/*
        查询下一节点对应的角色
     */
function queryNodeRole(objNo,objKey){
    var taskId;
    var nodeConf;
    InitFuncCodeRequest_({
        data: {
            FunctionCode: 'WS_FLOW_EXECUTION_QUERY_NEXT_ACTIVITY',
            objNo: objNo,
            objKey: objKey
        },
        async:false,
    ...
    nodeConf = queryNodeConfByTaskId(taskId);
    return nodeConf;

}

function queryNodeConfByTaskId(taskId){
    var nodeConf="";
    if (typeof(taskId) != "null" && taskId != null){
        InitFuncCodeRequest_({
            data: {taskId: taskId, FunctionCode: "NODE_ROLE_CONFIG_QUERY_ROLE_ID_BY_TASK_ID"},
            async: false, successCallBack: function (jdata) {
                if (jdata.code == RESULT_CODE.SUCCESS_CODE) {
                    console.info(jdata)
                    if(jdata.data!=null && jdata.data.ROLE_ID){
                        nodeConf= jdata.data;
                    }
                }
            }
...
function queryRoleId(flowKey){
    var roleId="";
    if (typeof(flowKey) != "null" && flowKey != null){
        InitFuncCodeRequest_({
            data: {flowKey: flowKey, FunctionCode: "NODE_ROLE_CONFIG_QUERY_ROLE_ID"},
            async: false, successCallBack: function (jdata) {
                if (jdata.code == RESULT_CODE.SUCCESS_CODE) {
                    if(jdata.data!=null && jdata.data.ROLE_ID){
                        roleId= jdata.data.ROLE_ID;
                    }
                }
            }
        });
 ...
 * 毫米格式化成年-月-日 时:分:秒
 */
function changeDateFormatExt(cellval) {
    if (cellval != null) {
        var date = new Date(parseInt(cellval, 10));
        var month = date.getMonth() + 1 < 10 ? "0" + (date.getMonth() + 1) : date.getMonth() + 1;
        var currentDate = date.getDate() < 10 ? "0" + date.getDate() : date.getDate();
 ...
 * 目标文件时间转换
 */
function changeTargetDateFormat(data, id) {
    var datas = data.data;
    ...
    for(var index in datas){
        ...
        for (var key in obj) {
            if (key == id) {
                var val = obj[key];
                var newDate = changeDateFormatExt(val);
                obj[key] = newDate;
            }
        }
 ...
 *  2020/8/19
 */
function  formatterUpperDataGrid(row,id){
    var rows = row.rows;
    ...
    for(var index in rows){
        ...
        //index必须整型，否则序号显示异常
        $('#'+id).datagrid('refreshRow', parseInt(index));
    }
}
...
/**
 *  功能描述: 驼峰转换下划线， 如：companyCode转换为COMPANY_CODE
 *  01379392(王大桥)
 *  2020/8/19
 */
...
/**
 *  功能描述:下划线转换驼峰，如：COMPANY_CODE转换为companyCode
 *  01379392(王大桥)
 *  2020/8/19
 */
...
function openTabMessage(url,name,id){
    var obj ={
        data :{
          'name':name,
          'id':id,
    ...
    if(typeof (BroadcastChannel) != "undefined"){
        var bc = new BroadcastChannel('open-tab-ne');
        bc.postMessage(obj.data);
    }else{

...
if(_b2=="list"){
    ...
    ps.bind('keypress', function(event) {
        if (event.keyCode == "13"){
            var psize = $(this).val();
            if (psize != "" && /^\d+$/.test(psize) && parseInt(psize) > 0) {
                _ae.pageSize = parseInt(psize);
...
_669.sortOrder=_66b.join(",");
    ...
    var col_ = {};
    for(var _i=0; _i < opts.columns[0].length; _i++){
        if(opts.columns[0][_i].field == _669.sortName){
            col_ = opts.columns[0][_i];
            break;
        }
    }
    if(opts.onBeforeSortColumn.call(_666,_669.sortName,_669.sortOrder,_669,col_)==false){
return;
}
$.extend(opts,_669);
var dc=_668.dc;
...
/*? 行编辑样式*///cc.push("<td class=\"datagrid-td-rownumber\"><div class=\"datagrid-cell-rownumber\">"+_7cd+"</div></td>");
    ...
    var _c = '';
    var $c = _7cc.__editing ? "<img src='/css/icons/cancel.png' class='e_cancel_' style='width:14px;'/><img src='/css/icons/accept.png' class='e_ok_' style='width:16px;' />" : ''
    cc.push("<td class=\"datagrid-td-rownumber\"><div class=\"datagrid-cell-rownumber"+_c+"\">"
        + ($c ? $c : _7cd) +"</div></td>");
    /*?*/
}
...
var xhr = $.ajax({async:false, type:opts.method,url:opts.url,header:opts.headers,xhrFields:opts.xhrFields,crossDomain:opts.crossDomain,data:_7ff,dataType:"json",success:function(data){
...
var txt = '', val = '';
    if(row[opts.textField]){
        txt = row[opts.textField]+'' || '';
    }
    if(row[opts.valueField]){
        val = row[opts.valueField]+'' || '';
    }
    if(opts.filterField && Object.prototype.toString.call(opts.filterField) == '[object Array]'){
        for(var i=0; i<opts.filterField.length; i++){
           var v = row[opts.filterField[i]]+'' || '';
           if(v.toLowerCase().indexOf(q.toLowerCase())>=0){
               return true;
           }
        }
    }
    return txt.toLowerCase().indexOf(q.toLowerCase())>=0 || val.toLowerCase().indexOf(q.toLowerCase())>=0;

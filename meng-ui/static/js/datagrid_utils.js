...
(function ($) {
    ...
    $.fn.GatewayDataGrid = function (options) {
        var opts = $.extend({}, $.fn.MyDataGrid.defaults, options);
        //请求参数中配置result 01374973
        opts.server = "gateway";
        var ArryList = [];
        ArryList[0]= opts.queryParams;
        opts.queryParams ={queryParam:ArryList};
        if (!validOptions_(opts)) { //验证必要参数
            return this;
        }
        if (opts.columns.param) {
    ...
    //扩展设置表格列columns的hidden属性可隐藏或显示对应的列，同时设置sortNum可以调整列的排序位置
    //如：hidden:true  ----为隐藏
    //如：hidden:false ----为显示
    //如：columns下的hidden属性没有  ----为显示
    // sortNum的值参考功能号的排序
    //可参考《生产准备单列表页面》
    //黎少环 80004886
    $.fn.GatewayDataGridHideCol = function (options) {
        var opts = $.extend({}, $.fn.MyDataGrid.defaults, options);
        //请求参数中配置result 01374973
        opts.server = "gateway";
        var ArryList = [];
        ...
        if (opts.columns.param) {
            asynBindDataGridHideCol(this, opts);
        } else {
            BindDataGrid(this, opts, '');
        }
    };
    ...
    //此方法配合  GatewayDataGridHideCol使用
    //黎少环 80004886
    function asynBindDataGridHideCol(thiz, opts) {
        opts._params = {};
        opts._params.url = opts.columns.url;
        opts._params = $.extend({}, opts._params, opts.columns.param);
        var url = opts.columns.url ? opts.columns.url : "/api/v1/system/dgcols";
        ...
        AjaxCall_(url, data, function (result) {
                ...
                if (opts.columns.alter) {
                    var kk=0;
                    $.each(columns_, function (k, v) {
                        if (opts.columns.alter[v.field]&&!opts.columns.alter[v.field].hidden) {
                            columns_new[kk]=$.extend({}, columns_[k], opts.columns.alter[v.field]);
                            //设置自定义的列宽
                            if(opts.columns.alter[v.field].width){
                                columns_new[kk].width=opts.columns.alter[v.field].width;
                            }
                            //自定义列排序
                            if(opts.columns.alter[v.field].sortNum){
                                columns_new[kk].sortNum=opts.columns.alter[v.field].sortNum;
                            }
                            kk++;
                        }
                    });
                ...
                // //按sortNum的值重新对列递增排序
                columns_new.sort(function(a,b){
                    return a.sortNum-b.sortNum;
                });
                opts.ocolumns = opts.columns;
                opts.columns = columns_new;
                BindDataGridHideCol(thiz, opts, functionCode);
            },
            function (errorThrown) {
                MsgAlert({content: errorThrown, type: 'error'});
            });
    ...
    function BindDataGrid(thiz, opts, fcode) {
        ...
        var loader = opts.loader || function (data, _800, _801) {
            var opts = $(this).datagrid("options");
            if (!opts.url) {
                return false;
            }
            if(gateWayServer === 'gateway'){
                data =  JSON.stringify(data);
            }else{
                //解决同一页面网关和正常请求切换时异常
                if(opts.server === 'gateway'){
                    opts.headers && delete opts.headers
                }
            }

            $.ajax({
                type: opts.method, url: opts.url, headers: opts.headers,
                xhrFields:opts.xhrFields,crossDomain:opts.crossDomain,
                data: data, dataType: "json", success: function (jdata) {
                    if (!AjaxSuccess(jdata)) {
                        return;
                    }
                    var list;
                    var data;
                    if(gateWayServer === 'gateway' &&  jdata.success === true ){
                        list = isArray(jdata.obj.list) ? jdata.obj.list : null;
                        data = {total: jdata.obj.total || 0, rows: list || [], data: jdata.obj.list};
                    }else if(gateWayServer === 'gateway'){
                        MsgAlert({content: jdata.msg, type: 'error'});
                        return;
                    }

                    if (gateWayServer != 'gateway' &&  jdata.code != 200) {
                        MsgAlert({content: jdata.msg, type: 'error'});
                        return;
                    }else if (gateWayServer != 'gateway'){
                        list = isArray(jdata.data) ? jdata.data : jdata.data['DG_ROWS'];
                        data = {total: jdata.total || 0, rows: list || [], data: jdata.data};
                    }
        ...
        opts.firstLoad = typeof(opts.firstLoad) == 'undefined' ? true : opts.firstLoad;
        opts.rownumbers = typeof(opts.rownumbers) != 'undefined' ? opts.rownumbers : true;

        var $col = [];
        if (fcode) {
            $col = [handleColumns_(columns_)];
        } else {
            $col = Object.prototype.toString.call(columns_[0]) == '[object Array]' ? columns_ : [columns_];
        }
        /*if(opts.enableLineEdit){
            $col.unshift({field: "__", title: "", width: 60});
        }*/
        var opt_ = {
            columns: $col,
            striped: true,
            pagination: typeof(opts.pagination) != 'undefined' ? opts.pagination : true,
            singleSelect: true,//typeof(opts.singleSelect) != 'undefined' ? opts.singleSelect : true,
            checkOnSelect: false,//typeof(opts.checkOnSelect) != 'undefined' ? opts.checkOnSelect : false,
            selectOnCheck: false,//typeof(opts.selectOnCheck) != 'undefined' ? opts.selectOnCheck : false,
            iconCls: typeof(opts.iconCls) != 'undefined' ? opts.iconCls : 'icon-application_view_detail',
            loadMsg: typeof(opts.loadMsg) != 'undefined' ? opts.loadMsg : "加载中...",
            rownumbers: opts.rownumbers,
            loader: loader,
            onBeforeLoad: function (param) {
                var flag = opts.firstLoad ? true : false;
        ...
        if (opts.data) {
            ...
            opt_.url = ENVGlobal.gateWay + opts.path;
            var headers = {
    ...
    //此方法配合  asynBindDataGridHideCol
    //黎少环 80004886
    function BindDataGridHideCol(thiz, opts, fcode) {
        //解决同一页面网关和正常请求切换时异常
        var gateWayServer = opts.server;
        var columns_ = opts.columns;
        opts.identity = opts.identity || thiz.selector.replace("#", '');
...
function onGateWaySearch_(identity, fromId) {
    fromId = fromId || "#ffSearch";
    var dgopt = getDgOpts(identity);
    if (!dgopt)
        return null;
    ...
    var url = dgopt.url;
    if ($dg && url) {
        var interParams = $(fromId).serializeObject();
        for(var key in interParams){
            dgopt.queryParams.queryParam[0][key] = interParams[key];
        }
...
function deleteFile(pkid) {
    if (!confirm("是否刪除"))
        return;
    InitFuncCodeRequest_({
        data: {pkid: pkid, FunctionCode: 'ATTACHMENT_DEL'}, async: false,
        successCallBack: function (jdata) {
            if (jdata.code == 200) {

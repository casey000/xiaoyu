...
;(function( $, window, undefined ){
    ...
    var _ie6 = window.ActiveXObject && !window.XMLHttpRequest,
        ...
        /*dom_array由zhf添加，解决关闭子页面后，焦点不回归父页面问题*/
        dom_array=[],
        dialogTpl =
            ...
            '<a class="ui_min" href="javascript:void(0);" title="Minimize"><b class="ui_min_b"></b></a>' +
            '<a class="ui_max" href="javascript:void(0);" title="Maximize"><b class="ui_max_b"></b></a>' +
            '<a class="ui_res" href="javascript:void(0);" title="Restore"><b class="ui_res_b"></b><b class="ui_res_t"></b></a>' +
            '<a class="ui_close" href="javascript:void(0);" title="Close(Esc Key)">\xd7</a>' +
            '</div>' +
            '</div>' +
            '</td>' +
            '</tr>' +
    ...
    lhgdialog.fn = lhgdialog.prototype =
        {
            ...
            show: function( args )
            {
                //this.DOM.wrap[0].style.display = 'block';//1
            ...
            hide: function( args )
            {
                //this.DOM.wrap[0].style.display = 'none';//1
            ...
            {
                /*
                 * 以下部分同zhf添加，解决关闭子页面后，焦点不回归父页面问题
                 * 方法：把打开的对话框按层级依次存入数组，关闭后，上一层获得焦点
                 */
                var cur_focus = lhgdialog.focus;
                if(cur_focus!= null){
                    var cur_id=cur_focus.config.id,flag = false;
                    for(var len=dom_array.length-1;len>=0;len--){
                        if(cur_id==dom_array[len].config.id){
                            flag = true;
                            break;
                        }
                    }
                    if(!flag){
                        dom_array.push(cur_focus);
                    }else{
                        dom_array.splice(len,1);
                        dom_array.push(cur_focus);
                    }
                }
                /*zhf add end*/
            ...
            close: function()
            {
                ...
                if(this.config != undefined && dom_array[dom_array.length-1]!= undefined && typeof dom_array[dom_array.length-1].config =="object" && this.config.id == dom_array[dom_array.length-1].config.id){
                    if(dom_array.length-2>=0){
                        dom_array[dom_array.length-2].zindex();
                    }
                    if(dom_array.length != 0){
                        dom_array.length-=1;  
                    } 
                }
                /*zhf add end*/

                /* add by wxw 解决person_select.js中回调函数实际并没有执行的问题*/
                if(this.config != undefined && typeof this.config.callback == "function"){
                    this.config.close()
                }
                /*wxw add end*/
                ...
                else
                {
                    ...
                    DOM.main[0].style.cssText = 'height:auto';
            ...
            _iframe: function( url )
            {
                ...
                that._frmTimer = setTimeout(function(){
                    if(config.method == "post" || config.method == "POST"){
                        ...
                        var	$form = $("<form method='post' target='" + iframe.name +"' style='display:none' action='" + queryString[0] + "'></form>");
                        if(config.postData != null){
                            $.each(config.postData, function(key, val){
                                var ipt = $("<input name='" + key + "'/>");
                                ipt.val(val);
                                $form.append(ipt);
                            });
                        }
                        ...
                        if(queryString.length >= 2){
                            ...
                            $.each($params, function(i, val){
                                var idx = val.indexOf("=");
                                var ipt = $("<input name='" + val.substr(0,idx) + "'/>");
                                ipt.val(val.substr(idx + 1));
                                $form.append(ipt);
                            });
                        }
                        $content.append($form);
            ...
            _addEvent: function()
            {
                ...
                $(window).bind('resize', that._winResize);//zhf修改_$top修改为$(window),解决非ie浏览器未定义化错误
                ...
                DOM.wrap.bind('click', function(event){
                     ...
                     */
                    for(var len=dom_array.length-1;len>=0;len--){
                        if(dom_array[len].config == undefined){
                            continue;
                        }else if(that.config.id==dom_array[len].config.id){
            ...
            _removeEvent: function()
            {
                ...
                DOM.wrap.unbind();
                DOM.title.unbind();
                $(window).unbind('resize', that._winResize);//zhf修改_$top修改为$(window),解决非ie浏览器未定义化错误
            }
        };
    ...
    lhgdialog.setting =
        {
            ...
            max: true,                  // 是否显示最大化按钮
            min: true,                  // 是否显示最小化按钮
            zIndex: 2100,				// 对话框叠加高度值(重要：此值不能超过浏览器最大限制)
...
//		title: '提问',
     ...
     * @param	{String}	默认值
     */
    lhgdialog.cPrompt = function(content, yes, value, parent )
    {
        value = value || '';
        var input;

...
(function ($, undefined) {
    ...
    $.fn.gantt = function (options) {
        ...
        var settings = {
            ...
            onRightClick: function (data) { return; },
        ...
        var core = {
            ...
            leftPanel: function (element) {
                ...
                var ganttLeftPanel = $('<div class="leftPanel"/>')
                    ...
                    .css("height", tools.getCellSize() * 2));
                    ...
                    for (i = 0, len = range.length; i < len; i++) {
                        ...
                        if (rgetDay !== getDay) {
                            ...
                            dayArr.push(
                                ...
                                '<div class="fn-label">' + year+'-' + settings.months[month] +'-'+ day.getDate()+ '</div></div>');
                            dowArr.push(
            ...
            createProgressBar: function (label, desc, classNames, dataObj) {
                label = label || "";
                var bar = $('<div class="bar"><div class="fn-label">' + label + '</span></div></div>')
                        .data("dataObj", dataObj);
                if(classNames == 'ganttFlight')
                {
                    var labelArr
                    ...
                    for(var i in labelArr){
                        ...
                        if( i == 0){
                            sHtml += '<div style="position: relative"><span style="float: left;width: 50%;text-align: left">'+ labelArr[i] +'</span>'
                        }
                        if( i == 1){
                            sHtml += '<span style="color: red; float: left;position: absolute;right: 39%; ">'+ labelArr[i] +'</span>'
                        }
                        if( i == 2){
                            sHtml += '<span style="float: right;width: 50%;text-align: right">'+ labelArr[i] +'</span></div>'
                        }
                    }
                     bar = $('<div class="bar"><div class="fn-label">' + sHtml + '</div></div>')
                        .data("dataObj", dataObj);
                }
                if (desc) {
                    if(classNames !== 'ganttFlight'){
                    ...
                    bar
                        .mouseenter(function (e) {
                            // let type = 'STOP_PRO_REA'
                            // let url = "/api/v1/mcc/visualBoard/selectVisualBoardRemark?flightId=1111"
                ...
                if(classNames !== 'ganttFlight'){
                    bar.bind("contextmenu", function(e){
                        e.stopPropagation();
                        settings.onRightClick($(this).data("dataObj"));
                        // console.log(this)
                        // console.log(desc)
                        // console.log(dataObj)
                        // console.log(e.which);
                ...
                $.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage &&
                        ...
                        $.each(entry.values, function (j, day) {
                            ...
                            // **Hourly data**
                            case "hours":
								var fromTime = tools.dateDeserialize(day.from)
                                dFrom = tools.genId(tools.dateDeserialize(day.from), element.scaleStep);
                                from = $(element).find('#dh-' + dFrom);
								var  min =  new Date (fromTime).getMinutes();
								console.log(from.data)
                                dTo = tools.genId(tools.dateDeserialize(day.to), element.scaleStep);
								var  min2 =  new Date (tools.dateDeserialize(day.to)).getMinutes();

                                to = $(element).find('#dh-' + dTo);
                                cFrom = from.data("offset") + min/60*44;
                                cTo = to.data("offset") +  min2/60*44;
            ...
            genId: function (t) { // varargs
                ...
                case "hours":
                    ...
                    if (arguments.length >= 2) {
						// console.log(Math.floor(t.getHours() / arguments[1]))

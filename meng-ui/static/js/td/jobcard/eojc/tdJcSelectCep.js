...
function sureButton_onClick() {
    ...
    if (radioValue == "SB") {
        ...
        if (cepFilePath) {
            ...
            if (fleet == 'A350') {
                openModalDialog("/views/td/jobcard/eojc/SelectS1000DTask.shtml", params);
            } else {
                openModalDialog("/views/td/jobcard/eojc/SelectATA2200Task.shtml", params);
            }
        } else {
            if (fleet == 'A350') {
                openModalDialog("/views/td/jobcard/eojc/SelectS1000DTask.shtml", params);
        ...
        if (!cepFilePath) {
            MsgAlert({content: "该工卡未创建工序，请选取SB或者自编工序！", type: 'error'});
        ...
        // 调成后台JCHandle处理类，并返回页面所需的参数
        ajaxLoading();
        InitFuncCodeRequest_({
            data: {jcPkid: jcPkid, FunctionCode: 'TD_JC_EOJC_UDFJOBCARD'},

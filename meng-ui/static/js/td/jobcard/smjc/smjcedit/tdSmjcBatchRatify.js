...
function i18nCallBack() {
    ...
    InitFuncCodeRequest_({
        data: {
            domainCode: "DA_FLEET,TD_JC_STATUS,YESORNO,TD_JC_FROM_TO,TD_JC_MODEL,TD_JC_STATE,TD_JC_EXTEND,TD_JC_SMJC_APPROVED_BY," +
            "TD_JC_SMJC_REVIEWED_BY",
        ...
        successCallBack: function (jdata) {
            if (jdata.code == RESULT_CODE.SUCCESS_CODE) {
                ...
                $('#approvedBy').combobox({
                    panelHeight: '100px',
                    data: jdata.data.TD_JC_SMJC_APPROVED_BY,
                    valueField: 'VALUE',
                    textField: 'TEXT',
...
function InitDataGrid() {
    ...
    $("#dg1").MyDataGrid({
        ...
        sortable: true,
        singleSelect: true,
        queryParams: {status: "RATIFY", approvedBy: user.accountId},
        ...
        contextMenus: [
            ...
            //
            //         common_add_edit_({
            //             identity: '', isEdit: '', width: 520, height: 300, title: $.i18n.t('审批'),
            //             param: {
            //                 roleId: '',
            //                 pkIds: rowData.PKID,
            ...
            //     id: "m-ratifytrunback",
            //     i18nText: "退回",
            //     auth: "TD_JC_SMJC_RATIFY_TURNBACK",
            //     onclick: function () {
            //         var rowData = getDG('dg1').datagrid('getSelected');
            //         if (!rowData.PKID) {
            //             MsgAlert({content: "请选择数据！", type: 'error'});
            ...
            // },
            {
                id: "m-previewpdf",
                i18nText: "PDF预览",
                auth: "TD_JC_SMJC_RATIFY_PDFVIEW",
                onclick: function () {
                    var rowData = getDG('dg1').datagrid('getSelected');
                    if (!rowData.PKID) {
                        MsgAlert({content: "请选择数据！", type: 'error'});

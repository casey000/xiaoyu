...
function checkRefData(refData) {
    ...
    ajaxLoading();
    InitFuncCodeRequest_({
        data: {pkid: jcPkid, refData: refData, FunctionCode: 'TD_JC_SMJC_CHECKREFDATA'},
        async: false,
        successCallBack: function (jdata) {
            ajaxLoadEnd();
            if (jdata.code == RESULT_CODE.SUCCESS_CODE) {
...
function translateManual(pkid) {
    ...
    ShowWindowIframe({
        ...
        title: title_,
        param: {pkid: pkid},
        url: "/views/td/jobcard/smjc/smjcedit/tdSmjcTranslateList.shtml"
    });
}
...
function manualToJobcard() {
    ...
    ajaxLoading();
    InitFuncCodeRequest_({
        data: {jcPkid: jcPkid, manualFilePath: rowData.MANUAL_FILE_PATH, FunctionCode: 'TD_JC_TOJOBCARD'},
        successCallBack: function (jdata) {
            ajaxLoadEnd();

...
function i18nCallBack() {
    ...
    InitFuncCodeRequest_({
        data: {
            domainCode: "DA_FLEET,TD_JC_STATUS,YESORNO,TD_JC_FROM_TO,TD_JC_MODEL,TD_JC_STATE,TD_JC_EXTEND,TD_JC_SMJC_PROOF_BY,TD_JC_SMJC_WRITER",
        ...
        successCallBack: function (jdata) {
            if (jdata.code == RESULT_CODE.SUCCESS_CODE) {
                ...
                $('#proofBy').combobox({
                    panelHeight: '100px',
                    data: jdata.data.TD_JC_SMJC_PROOF_BY,
                    valueField: 'VALUE',
                    textField: 'TEXT',
...
function InitDataGrid() {
    ...
    $("#dg1").MyDataGrid({
        ...
        sortable: true,
        singleSelect: true,
        queryParams: {status: "PROOF", proofBy: user.accountId},
        ...
        contextMenus: [
            {
                id: "", i18nText: "编辑", auth: "TD_JC_SMJC_PROOF_EDIT",
                onclick: function () {
                    ...
                    ShowWindowIframe({
                        ...
                        height: "650",
                        title: title_,
                        param: {pkid: rowData.PKID, type: "reEdit", from: "CUS"},
                        url: "/views/td/jobcard/smjc/smjcedit/tdSmjcEditDetail.shtml"
                    });
                }
            },
            {
                id: "", i18nText: "校对", auth: "TD_JC_SMJC_PROOF_EDIT",
                onclick: function () {
                    var rowData = getDG('dg1').datagrid('getSelected');
                    if (!rowData.PKID) {
                        MsgAlert({content: "请选择数据！", type: 'error'});
                    ...
                    common_add_edit_({
                        ...
                        param: {
                            roleId: '',
                            pkIds: rowData.PKID,
                            FunctionCode_: 'TD_JC_ALL_SMJC_BATCH_SUBMIT',
                            processNode: 'PROOF',
                            submitType: 'doSubmit',
                            successCallback: reload_
                        },
                        url: "/views/td/jobcard/cmjc/td_Cmjc_work_flow_batch_deal.shtml"
                    });
                }
            },
            {
                id: "", i18nText: "退回", auth: "TD_JC_SMJC_PROOF_TURNBACK",
                onclick: function () {
                    var rowData = getDG('dg1').datagrid('getSelected');
                    if (!rowData.PKID) {
                        MsgAlert({content: "请选择数据！", type: 'error'});
                    ...
                    common_add_edit_({
                        ...
                        param: {
                            roleId: '',
                            pkIds: rowData.PKID,
                            FunctionCode_: 'TD_JC_ALL_SMJC_BATCH_RETURN',
                            submitType: 'doReturn',
                            successCallback: reload_
                        },
                        url: "/views/td/jobcard/cmjc/td_Cmjc_work_flow_batch_deal.shtml"
        ...
        toolbar: [
            ...
            },
            {
                key: "COMMON_ADD",
                text: "批量退回",
                auth: "TD_JC_SMJC_PROOF_TURNBACK",
                handler: function () {
                    var rowData = getDG('dg1').datagrid('getChecked');
                    if (rowData.length == 0) {
                        MsgAlert({content: "请选择数据！", type: 'error'});

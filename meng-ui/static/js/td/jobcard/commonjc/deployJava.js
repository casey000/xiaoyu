var deployJava = function () {
    ...
    function versionCheckEx(query, version) {
        ...
        // if it is not explicit pattern but does not have update version then
        // need to append *
        ...
        // if query ends with ".", "_" then we want to strip it to allow match
        // of "1.6.*" to shorter form such as "1.6"
    ...
    function getWebStartLaunchIconURL() {
        ...
        try {
            // for http/https; use protocol less url; use http for all other
            // protocol
            return document.location.protocol.indexOf('http') != -1 ? imageUrl
                : 'http:' + imageUrl;
        } catch (err) {
     ...
     *
     * requestedJREVersion param is optional - if null, it will be treated as
     * installing any JRE version
     *
     * DT plugin for 6uX only knows about JRE installer signed by SUN cert. If
     * it encounter Oracle signed JRE installer, it will have chance of deadlock
    ...
    var rv = {
         ...
         * deployJava.js at: http://java.com/js/webstart.png
         *
         ...
         * strings are of the form #.#[.#[_#]], with the function returning as
         * much version information as it can determine, from just family
         * versions ("1.4.2", "1.5") through the full version ("1.5.0_06").
         *
         * Detection is done on a best-effort basis. Under some circumstances
         * only the highest installed JRE version will be detected, and JREs
         * older than 1.4.2 will not always be detected.
         */
        getJREs: function () {
            var list = [];
            ...
            if (this.debug) {
                for (var i = 0; i < list.length; ++i) {
                    log('[getJREs()] We claim to have detected Java SE '
                        + list[i]);
                }
            }
         ...
         *
         * The requestVersion string is of the form #[.#[.#[_#]]][+|*], which
         * includes strings such as "1.4", "1.5.0*", and "1.6.0_02+". A star (*)
         * means "any version starting within this family" and a plus (+) means
         * "any version greater or equal to this". "1.5.0*" * matches 1.5.0_06
         * but not 1.6.0_01, whereas "1.5.0+" matches both.
         *
         * installCallback is an optional argument which holds a reference to a
         * javascript callback function for reporting install status.
         *
        ...
        installJRE: function (requestVersion, installCallback) {
            ...
            if (this.isPluginInstalled()
                ...
                if (this.isCallbackSupported()) {
                    installSucceeded = this.getPlugin().installJRE(
        ...
        /**
         * returns true if jre auto install for the requestedJREVersion is
         * enabled for the local system; false otherwise
         *
         ...
         * deadlock when running with IE. This function is to guard against
         * this.
         */
        ...
        /**
         * returns true if jre install callback is supported callback support is
         * added since dt plugin version 10.2.0 or above
         */
         ...
         * not always possible, as the JRE installation may require the browser
         * to be restarted.
         *
         * installCallback is an optional argument which holds a reference to a
         * javascript callback function for reporting install status.
         *
         * In the best case (when the Deployment Toolkit plugin is present),
         * this function will immediately cause a progress dialog to be
         * displayed as the JRE is downloaded and installed.
         */
            ...
            if (this.isPluginInstalled() && this.isAutoInstallEnabled()) {
                ...
                if ((this.EAInstallEnabled == 'true')
                    ...
                    if (browser == 'MSIE') {
                        ...
                        location.href = constructGetJavaURL(((this.returnPage != null) ? ('&returnPage=' + this.returnPage)
                            : '')
         ...
         * minimumVersion is of the form #[.#[.#[_#]]], and is the minimum JRE
         * version necessary to run this applet. minimumVersion is optional,
         * defaulting to the value "1.1" (which matches any JRE). If an equal or
         * greater JRE is detected, runApplet() will call
         * writeAppletTag(attributes, parameters) to output the applet tag,
         * otherwise it will call installJRE(minimumVersion + '+').
         *
         * After installJRE() is called, the script will attempt to detect that
         * the JRE installation has completed and begin running the applet, but
         * there are circumstances (such as when the JRE installation requires a
         * browser restart) when this cannot be fulfilled.
         *
         * As with writeAppletTag(), this function should only be called prior
         * to the web page being completely rendered. Note that version
         * wildcards (star (*) and plus (+)) are not supported, and including
         * them in the minimumVersion will result in an error message.
         */
            ...
            if (matchData != null) {
                ...
                if (browser != '?') {
                    if (this.versionCheck(minimumVersion + '+')) {
                        ...
                        // after successful install we need to refresh page to
                        // pick
                        // pick up new plugin
                        this.refresh();
         ...
         * where both attributes and parameters are associative arrays. Each
         * key/value pair in attributes becomes an attribute of the applet tag
         * itself, while key/value pairs in parameters become <PARAM> tags. No
         * version checking or other special behaviors are performed; the tag is
         ...
         * (among those detected by getJREs()). The versionPattern string is of
         * the form #[.#[.#[_#]]][+|*], which includes strings such as "1.4",
         * "1.5.0*", and "1.6.0_02+". A star (*) means "any version within this
         * family" and a plus (+) means "any version greater or equal to the
         * specified version". "1.5.0*" matches 1.5.0_06 but not 1.6.0_01,
         * whereas "1.5.0+" matches both.
         *
         ...
         * compares only as much of the version information as could be
         * detected, so versionCheck("1.5.0_06+") would return true in in this
         * case.
         *
        ...
        versionCheck: function (versionPattern) {
            ...
            if (matchData != null) {
                ...
                for (var i = 1; i < matchData.length; ++i) {
                    ...
                    // Fx sets 'undefined', IE sets '' string for unmatched
                    // groups
                ...
                if (patternArray[patternArray.length - 1] == '+') {
                    ...
                    // versionPattern does not include all four version
                    // components
                    // and does not end with a star or plus, it will be treated
                    // as
         ...
         * minimumVersion can be detected. minimumVersion is optional, and if
         * not specified, '1.4.2' will be used. (Versions earlier than 1.4.2 may
         * not be detected.)
         */
        isWebStartInstalled: function (minimumVersion) {
            ...
            if (matchData != null) {
                ...
                log('[isWebStartInstaller()] Invalid minimumVersion argument to isWebStartInstalled(): '
                    + minimumVersion);
        ...
        // if machine has no JRE 7 or above, we will try to auto-install and
        // then launch
        ...
        launchWebStartApplication: function (jnlp) {
            ...
            if (document.documentURI) {
                jnlpDocbase = encodeURI(document.documentURI);
            }

            // fallback to document.URL if documentURI not available
            if (jnlpDocbase == null) {
                jnlpDocbase = encodeURI(document.URL);
            }
            ...
            if (document.body == 'undefined' || document.body == null) {
                ...
                // go back to original page, otherwise current page becomes
                // blank
        ...
        createWebStartLaunchButtonEx: function (jnlp, minimumVersion) {
            ...
            var url = 'javascript:deployJava.launchWebStartApplication(\''
                + jnlp + '\');';
         ...
         * button will ensure that an appropriate JRE is installed and then
         * launch the JNLP application. minimumVersion is of the form
         * #[.#[.#[_#]]], and is the minimum JRE version necessary to run this
         * JNLP application. minimumVersion is optional, and if it is not
         * specified, '1.4.2' will be used. If an appropriate JRE or Web Start
         * installation is detected, the JNLP application will be launched,
         * otherwise installLatestJRE() will be called.
         *
         * After installLatestJRE() is called, the script will attempt to detect
         * that the JRE installation has completed and launch the JNLP
         * application, but there are circumstances (such as when the JRE
         * installation requires a browser restart) when this cannot be
         * fulfilled.
         */
        ...
        launch: function (jnlp) {
            /*
             * Using the plugin to launch Java Web Start is disabled for the
             * time being
             */
        ...
        compareVersionToPattern: function (version, patternArray, familyMatch,
            ...
            if (matchData != null) {
                ...
                if (minMatch) {
                    // minimum version match, return true if what we have
                    // (installed)
                    // is greater or equal to what is requested. false
                    // otherwise.
                    for (var i = 0; i < l; ++i) {
                    ...
                    if (familyMatch) {
                        ...
                        // exact match
                        // result and patternArray needs to have exact same
                        // content
        ...
        testUsingActiveX: function (version) {
            ...
            // the check needs to be in place here - cannot even pass
            // ActiveXObject

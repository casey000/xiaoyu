...
function add_aircraft(acId, acType, tail, status, topAcType) {
    ...
    //设置acId的值
    _node.find("td:eq(1)").find(":input:eq(0)").val(acId);
    ...
    //添加准备单按钮
    if ($("#survey_select_prodPrep_add").attr("disabled")) {
        $("#survey_select_prodPrep_add").attr("disabled", false);
        console.log($("#survey_select_prodPrep_add"));
        $("#survey_select_prodPrep_add")[0].style.cursor = "pointer";
    }
 ...
 * @param _obj
 */
function delete_aircraft_tr(_obj) {

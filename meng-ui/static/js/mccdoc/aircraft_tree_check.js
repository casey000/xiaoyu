...
function fillSelectedDataBefore() {
    ...
    //选中已经选择过的数据
    $.each($("td[field='type']"), function (i, v) {
        if (i != 0) {
            var acNo = $(v).find("div span.tree-title");
            var arrAcNos = acNos.split(",");
            if (acNo.text() != "" && $.inArray(acNo.text(), arrAcNos) > -1) {
                $(acNo).find("input").prop("checked", true);
            }
        }
    });
}

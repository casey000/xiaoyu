var isOutsideDepartment = false;
var isRoutes = false;
$(function () {
    let cardTypeArray = new Array();
            ...
            success: function (obj, textStatus) {
                if(obj.code == '200' || obj.code == '202') {
                    ...
                    if (mrInfo.length>0){
                        ...
                        $("#approver_name").textbox("setValue", mrInfo.approverName+"(" + mrInfo.approver + ")");
                        $("#approver_id").val(mrInfo.approver);
                        $("#reportor_name").textbox("setValue", mrInfo.mrReportedByName+"(" + mrInfo.mrReportedBy + ")");
                        $("#reportor_id").val(mrInfo.mrReportedBy);
                        $("#station").textbox('setValue', mrInfo.station);
                        $("#requireDate").datetimebox('setValue', mrInfo.requiredDate);
                        let mrNum = "MR"+mrInfo.mrNum.substring(1,mrInfo.mrNum.length);
                        $("#mrNo").textbox("setValue",mrNum);
                        oldRequireDate = mrInfo.requiredDate;
    ...
    function initDefectData() {
        initAc();
        $('#tailOrEng').combobox({
            data: [{VALUE: 'Aircraft', TEXT: 'Aircraft'}],
            valueField: 'VALUE',
            textField: 'TEXT',
        ...
        $("#cardId").textbox('setValue', params.cardId);
    ...
    function initLMData(){
        ...
        if(userInfo.roleName){
            ...
            routes.push("飞机勤务员");
            routes.push("航线机务生产储备");
            routes.push("机务体系");
            routes.push("机务生产体系负责人");
            routes.push("航线放行工程师");
            routes.push("航线维修助理");
            routes.push("航线维修技术员");
            routes.push("航线维修技师");
            routes.push("航线维修经理");
            routes.push("技术支援助理工程师");
            routes.push("航线各处室负责人");
            routes.push("定检维修助理");
            routes.push("定检机务生产储备");
            routes.push("定检维修技术员");
            routes.push("定检维修技师");
            routes.push("定检放行工程师");
            routes.push("定检维修经理");
            routes.push("定检维修组负责人");
            $.each(userInfo.roleName.split(','),function(i,v){
                if("外部门" == v){
                    isOutsideDepartment = true;
                }
                if(routes.indexOf(v) > -1){
                    isRoutes = true;
                }
            });
        }
        if(isRoutes){
            cardTypeArray.push({VALUE: 'LM', TEXT: 'LM'});
            cardNoArray.push({VALUE: 'LM', TEXT: '航线勤务'});
        }
        if(isOutsideDepartment){
            cardTypeArray.push({VALUE: 'TET', TEXT: 'TET'});
            cardTypeArray.push({VALUE: 'AMR', TEXT: 'AMR'});
            cardTypeArray.push({VALUE: 'TOOL', TEXT: 'TOOL'});
            cardTypeArray.push({VALUE: 'STR', TEXT: 'STR'});
            cardTypeArray.push({VALUE: 'CR', TEXT: 'CR'});
            cardTypeArray.push({VALUE: 'FTW', TEXT: 'FTW'});
            cardTypeArray.push({VALUE: 'CT', TEXT: 'CT'});
            cardTypeArray.push({VALUE: 'EUR', TEXT: 'EUR'});
            cardTypeArray.push({VALUE: 'EDT', TEXT: 'EDT'});
            cardNoArray.push({VALUE: 'TET', TEXT: '培训航材需求'});
            cardNoArray.push({VALUE: 'AMR', TEXT: '模拟机航材需求'});
            cardNoArray.push({VALUE: 'TOOL', TEXT: '工具修理需求'});
            cardNoArray.push({VALUE: 'STR', TEXT: '结构修理需求预提'});
            cardNoArray.push({VALUE: 'CR', TEXT: '福永自主部件修理需求'});
            cardNoArray.push({VALUE: 'FTW', TEXT: '飞行训练机轮需求'});
            cardNoArray.push({VALUE: 'CT', TEXT: '化工品检测'});
            cardNoArray.push({VALUE: 'EUR', TEXT: '工程临时紧急需求预提'});
            cardNoArray.push({VALUE: 'EDT', TEXT: '工程测试开发PMA件等所需航材'});
        }
        $(".add_btn").attr("disabled", true);
        $(".satisfy").css('display','none');
        $('#tailOrEng').combobox({
            data: [{VALUE: 'Aircraft', TEXT: 'Aircraft'}, {VALUE: 'NA', TEXT: 'NA'}],
            valueField: 'VALUE',
            textField: 'TEXT',
            onLoadSuccess: function () {
                $('#tailOrEng').combobox('setValue', 'Aircraft');
        ...
        $('#cardType').combobox({
            ...
            textField: 'TEXT',
            onLoadSuccess: function () {
                $('#cardType').combobox('setValue', cardNoArray[0].VALUE);
                $("#cardNo").textbox('setValue',cardNoArray[0].TEXT);
            }
        });
    }
    // 初始化NRC数据
    function initNRCData(){
        ...
        $("#ser_acReg").combobox('setValue', params.acReg);
        $("#station").textbox('setValue', params.station||"SZX");
        $("#cardNo").textbox('setValue', params.cardNum);
    ...
    function initLineData(type){
        initAc();
        $('#tailOrEng').combobox({
            data: [{VALUE: 'Aircraft', TEXT: 'Aircraft'}, {VALUE: 'NA', TEXT: 'NA'},{VALUE: 'ENG', TEXT: 'ENG'}],
            valueField: 'VALUE',
            textField: 'TEXT',
            onLoadSuccess: function () {
                if(params.acReg=="null"){
                    $('#tailOrEng').combobox('setValue', 'ENG');
                }else{
                    $('#tailOrEng').combobox('setValue', 'Aircraft');
                }
            }
        ...
        $('#esnId').combobox('setValue',params.assetNum);
        $("#ser_acReg").combobox('setValue', params.acReg||'');
        $("#station").textbox('setValue', params.station||"SZX");
    ...
    function initItem(responItem){
        ...
        $.each(responItem, function (index, value) {
            ...
            let satisfaction = value.satisfaction=='1'?"满足":"不满足";
            let deleteBtn = '';
...
function selectPn(data,time) {
    $(`select[name=unit_${time}]`).empty();//每次查询件号时，把Unit清空
    var station = $("#station").textbox('getValue').replace(/(^\s*)|(\s*$)/g, "");
...
function getDefectMrPostData() {
    ...
    if(mrItems.length < 1){
        if($("#deleteSerialNumber").val().length < 1){
    ...
    mr["cardId"] = $("#cardId").textbox('getValue');
...
function get_post_data() {
    ...
    if(mrAddparams.tdWorkType == 'DEFECT' || mrAddparams.tdWorkType == 'DD'){
        mr["tdWorkType"] = "DEFECT";
    }else if(isOutsideDepartment){
        mr["tdWorkType"] = $("#cardType").combobox('getValue');
    }else{
    ...
    mr["workType"] =  $("#cardType").combobox('getValue');
    mr["cardId"] = mrAddparams.cardId || null;
    mr["cardNum"] =  $("#cardNo").textbox('getValue');
    mr["requiredDate"] = $("#requireDate").datetimebox("getValue");
    mr["description"] = $("#remark").val();
    mr["approver"] = $("#approver_id").val();
...
function saveMr() {
    ...
    if(workType == 'DEFECT'){
        ...
        postData = getDefectMrPostData();
    } else if(isUpdate && isDisabled && isHasItem && (mrItems.length < 1)) {

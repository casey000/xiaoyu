...
$(function(){
	...
	$("#add_cc_btn").click(function() {
		var params = {
			...
			"tlbId":$("#tlbId").val(),
			"docType":"1",
			"docNo":$("input[name='techLog.tlbNo']").val(),
			"acId":$("#acId").val(),
			"acNo":$("#acNo").val(),
			"fltId":$("#flightId").val(),
			"fltNo":$("#flightNo").text()
...
function tlbNoFlag(obj){
	...
	if($(obj).attr("checked")){
		$("#noTlbNo").val("1");
		$("input[name='techLog.tlbNo']").remove();
		$("#add_cc_btn").remove();
	}else{
		var oldTlbNo = $("#oldTlbNo").val();
		$("#noTlbNo").val("");
		var tlbInput = '<input type="text" class="number required" min="0" style="width: 90px" name="techLog.tlbNo" value="'+oldTlbNo+'" />';
		$("#tlbNoError").prepend(tlbInput);
		//有TLB NO就允许添加CC
		var ccBut = '<input type="button" class="btn" value="Add CC" id="add_cc_btn" />';
		$("#ccTd").prepend(ccBut);
	}
}

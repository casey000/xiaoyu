...
function editVerifyResultInfo(rowObject){
	P.$.dialog({
		...
		width : "500px",
		height : "350px",
		content : "url:" + basePath + "/tlb/defect_rm_fault_editVerifyResult.action?faultNo=" + rowObject.multipleFaults,
		close : function(){
			$(document).sfaQuery().reloadQueryGrid();
		}
	});
...
function showDefectNo(multipleFaults, rowObject){
	$.ajax({
        url : basePath+"/tlb/defect_multiplefaults_getDefectNoOfSameMultipleNo.action",
        type : 'post',
		cache: false,
		dataType: "json",
        data: {
        	'defectBaseInfo.multipleFaults' : multipleFaults
        },
        success : function(dataMsg) {
        	dataMsg = JSON.parse(dataMsg);
			if(dataMsg.ajaxResult == 'success'){
				...
				P.$.dialog({
					...
					content : 'url:' + basePath + "/tlb/defect/defect_multipleFaults.jsp"
				});
			}else{
				if (dataMsg.message != null && dataMsg.message != "") {

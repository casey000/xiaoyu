...
function editVerifyResultInfo(rowObject){
	P.$.dialog({
		...
		width : "500px",
		height : "350px",
		content : "url:" + basePath + "/tlb/tlb_rm_fault_editVerifyResult.action?faultNo=" + rowObject.repeatFaults,
		close : function(){
			$(document).sfaQuery().reloadQueryGrid();
		}
	});
...
function showTlbNo(repeatFaults, rowObject){
	$.ajax({
        url : basePath+"/tlb/tlb_repeatfaults_getTlbNoOfSameRepeatNo.action",
        type : 'post',
		cache: false,
		dataType: "json",
        data: {
        ...
        success : function(dataMsg) {
			...
			if(dataMsg.ajaxResult == 'success'){
				...
				P.$.dialog({
					...
					content : 'url:' + basePath + "/tlb/tlb_repeatFaults.jsp"
				});
			}else{
				if (dataMsg.message != null && dataMsg.message != "") {

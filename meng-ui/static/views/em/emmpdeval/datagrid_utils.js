...
$.extend($.fn.datagrid.methods, {
    ...
    doCellTip: function (jq, params) {
        function showTip(data, td, e) {
            if ($(td).text() == "")
                return;
            //console.info($(td.childNodes[0]).attr("class")=="datagrid-cell datagrid-cell-c1-TASK_DESC")
            //console.info($($(td).parent()).attr("field"))
            if ($(td.childNodes[0]).attr("class") == "datagrid-cell datagrid-cell-c1-TASK_DESC") {
                // console.info($(td).pre().text());
                data.tooltip.text($(td).prev().text()).css({
                    top: (e.pageY + 10) + 'px',
                    left: (e.pageX + 20) + 'px',
        ...
        var loader = opts.loader || function (_7ff, _800, _801) {
            ...
            $.ajax({
                type: opts.method, url: opts.url, data: _7ff, dataType: "json", success: function (jdata) {
                    ...
                    var list = isArray(jdata.data) ? jdata.data : jdata.data['DG_ROWS'];
                    var data = {total: jdata.total || 0, rows: list || [], data: jdata.data};

...
function saveBg() {
    ...
    //首检大组原则校验
    var isValidP =$('#fcBgPrinciple').combobox("isValid");
    if(!isValidP){
                ...
                if (jdata.code == RESULT_CODE.SUCCESS_CODE) {
                    ...
                    $(":checkbox").each(function () {
                        ...
                        if (is == "Y") { //恢复 note 编辑
                            ...
                            if ($(this).attr('name') == "ifRcNote" || $(this).attr('name') == "ifRcTbd") {
...
$(":checkbox").each(function () {
        ...
        $(this).change(function () {
            ...
            //重检TBD
            if (radioName == "ifRcTbd") {
                if (is) {
                    if (!confirm("切换将清空结构化数据，是否确认？")) {
                        $(this).prop("checked", ifRcTbd ? true : false);
                        $(this).removeAttr("checked");
                        return;
                    }
                } else {
...
function showResult(data){
    ShowWindowIframe({
        ...
        url: "/views/em/emeo/edit/tlComputeResult.shtml"
    });
}
...
function initRcForm(data) {
    ...
    for(var i = 0; i< sgList.length; i++) {
        ...
        for (var j = 0; j < vList.length; j++) {
            ...
            //部件和发动机的重检去掉SINCE
            /*var deadLineNode = $(":input[name='vo.complianceTimes[0].eoSub1[0].eoSub2[" + i + "].eoSub3[" + j + "].deadLine']");
            if(param.applyType == 'PART' || param.applyType == 'ENG'){
                var ifSince = vList[j].ifSince;
                var sinceType = vList[j].sinceType;
                if(ifSince && ifSince == 'Y'){
...
function appendTable(data) {
    ...
    str = str + "<td width='8%' style='border-right:none; border-top:none; border-bottom:none'>" +
        ...
        "name='vo.complianceTimes[0].eoSub1[0].note' rows='6' style='width:150px;'></textarea>" +
...
function appendRcTable(data) {
    ...
    for(var i = 0; i< sgList.length; i++){
        ...
        for(var j=0; j<vList.length; j++){
            str = str + "<tr>" +
                ...
                "</td>" +
                "<td width='20%' name='deadLine'>" +
                "截止日期：<input type='text' name='vo.complianceTimes[0].eoSubRc1[0].eoSubRc2["+i+"].eoSubRc3["+j+"].deadLine'/>" +
                "</td>"+
                "<td width='5%'>" +
                "<div>" +
    ...
    str = str + "<td width='8%' style='border-right:none; border-top:none; border-bottom:none'>" +
        ...
        "name='vo.complianceTimes[0].eoSubRc1[0].note' rows='6' style='width:150px;'></textarea>" +
        "<div class='errorMessage'></div>" +
        "</td>";
    str = str + "</tr>";
    console.log(str);
...
function appendRcResetTable() {
    ...
    var isTbd = $('#ifRcTbd').is(':checked');
    if(!ifCf){ //一次性EO无重检
        $('#appendTableRc').find('select').attr('disabled','disabled');
        $('#appendTableRc').find('input').attr('disabled','disabled');
        $('#rcNote').attr('disabled', 'disabled');
        $('#ifRcNote').attr('disabled', 'disabled');
        $('#ifRcTbd').attr('disabled', 'disabled');

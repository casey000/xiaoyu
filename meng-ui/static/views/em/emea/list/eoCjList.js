...
function openDetai(operation, pkid) {
    ...
    let datas = {eoPkid: iparam.pkid, jcType: "EAJC", FunctionCode: 'EM_EO_JC_RELATE_CONUT'};
    InitFuncCodeRequest_({
        async : false,
        data: datas,
        successCallBack: function (jdata) {
...
function reload_(eoPkid) {
    ...
    if(eojcCount == 0){
        ...
        $('.bxyjTd').show();
        $('#writeWarning').combobox('setValue', 'N');
        $('#leadTime').combobox({onlyview: true});
        saveBxjy();
    }
...
function findEoJcCard() {
    InitFuncCodeRequest_({
        async: false,
        data: {eoPkid: iparam.pkid, jcType: 'EAJC', FunctionCode: 'EM_EOJC_BY_EOPKID'},
        successCallBack: function (jdata) {
            if (jdata.code == 200) {
                if(jdata.data){
                    eojcCount = 1;
